...
Threads, visible messages, turns, per-turn source snapshots, the active tray, the previous successful tray, and active thread identity are stored in `conversations.sqlite3` inside the installed plugin directory. The database is a standard SQLite file produced by the packaged sql.js WASM runtime. Conversation history stores only visible user messages and assistant responses; source serialization is not copied into historical user messages. Existing V1 IndexedDB data is not migrated; this amendment starts a fresh SQLite store.
