...
export class SynthesisDatabase {
  ...
  async open(): Promise<void> {
    ...
    try {
      ...
      this.schemaDirty = ensureColumn(this.db, "turn_usage", "reasoning_tokens", "INTEGER") || this.schemaDirty;
    } catch (error) {
      this.db?.close();
      this.db = null;
      throw error;
  ...
  async load(defaultModel: SynthesisModel = DEFAULT_MODEL, defaultReasoningEffort: ReasoningEffort = DEFAULT_REASONING_EFFORT, legacyModel?: unknown): Promise<PersistedState> {
    ...
    const turnUsage = rowObjects(db.exec("SELECT turn_id, input_tokens, output_tokens, reasoning_tokens, total_tokens, cached_input_tokens, cache_write_tokens, usage_json FROM turn_usage ORDER BY turn_id")).map((row): TurnUsage => {
      const usageJson = asNullableString(row.usage_json);
      let recoveredReasoning: number | null = null;
      if (usageJson) {
        try {
          const parsed = JSON.parse(usageJson) as { output_tokens_details?: { reasoning_tokens?: unknown } };
          recoveredReasoning = typeof parsed.output_tokens_details?.reasoning_tokens === "number" && Number.isFinite(parsed.output_tokens_details.reasoning_tokens) ? parsed.output_tokens_details.reasoning_tokens : null;
        } catch { /* Preserve malformed provider payloads without inventing a value. */ }
      }
